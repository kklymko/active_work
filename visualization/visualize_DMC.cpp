...
vector<double> propagation(vector<double>& x, int N, double L, double tint, gsl_rng* rng, vector<double>& theta,vector<double>& y,double lams,double Pe)
{
    ...
    //run lammps and have it output the trajectories or the observable. It might make more sense for it to just output the observable since we know that it works. in propagation have lammps dump trajectories and then we compute the local density as flux. THis is also where theentropy production would go. 
  	double h = 0.00005; //think this is the timestep
...
//The observable is the active work. We take the tagged particle to be the zeroth one. and look at only the x direction. 
            ...
            //fluxp[j]+=(Pe*cos(theta[j]))*(h / gamma * f[j][0]+ noise_0+Pe*cos(theta[j])*h)+Pe*sin(theta[j])*(h / gamma * f[j][1]+ noise_1+Pe*sin(theta[j])*h);
            //fluxp+=0.0;
...
{       int nprocs,rank;
	...
	int N = 20.0; //number of particles in the model
	...
	double tint = 0.0025, tobs =10.0;//tried 700 try twice as long with a million walkers
	int Ntint =4000;
//	int Ntint = tobs/tint;
  ...
  //variables below for visualization

  vector< vector<int> > history(Nw);
	vector< vector<int> > oldhistory(Nw);
	vector<double> temp1(N),temp2(N),temp3(N);  

  int print_increment = 10; //how often do we print the movie?
  int visualize = 1; // make 1 for movie, anything else for no movie
...
*/		}
	...
	fclose(result);
        ...
        for(i=0;i<Nw;i++){
				for(j=0;j<Ntint;j++){
     			if(j%10==0){	
						fprintf(historytable,"%d	%d\n",(int)(j/print_increment),history[i][(int)(j)]);
					}
				}
				}
        fclose(historytable);
