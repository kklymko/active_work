/*This code will run the dynamics for run and tumble particles (ABPs) and run cloning calculations from the paper "Entropy production fluctuations encode collective behavior in active matter" by Trevor GrandPre, Katie Klymko, Kranthi K. Mandadapu, and David T. Limmer (2020).
...
The inputs are lambda, self-propulsion v, the tumble rate, and time of system in that order. In order to compile, you will need GNU scientific library. */
...
//The observable is the active work. We take the tagged particle to be the zeroth one. and look at only the x direction. 
            ...
            fluxp+=(Pe*cos(theta[j]))*(h / gamma * f[j][0]+ noise_0+Pe*cos(theta[j])*h)+(Pe*sin(theta[j]))*(h / gamma * f[j][1]+ noise_1+Pe*sin(theta[j])*h+2*Pe*sin(theta[j])*h*lams/N);
